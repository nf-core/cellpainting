#!/usr/bin/env python3
"""
Generate load_data.csv for CellProfiler illumination application.

This script generates load_data.csv files for the illumination application step,
which uses original multi-channel images plus illumination correction files.
...
def collect_and_group_files(images_dir: str, illum_dir: str) -> Dict:
    """
    Collect and group image files and illumination files by (plate, well, site).
...
def main():
    ...
    print(f"\n{'='*60}", file=sys.stderr)
    print(f"CellProfiler Illumination Application CSV Generator", file=sys.stderr)
    ...
    try:
        ...
        grouped = collect_and_group_files(args.images_dir, args.illum_dir)
